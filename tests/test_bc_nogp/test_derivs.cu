...
int main(void)
{
    ...
    twodads::stiff_params_t stiff_params(0.1, Lx, Ly, 0.1, 0.0, Nx, My / 2 + 1, tlevs);

    {
        slab_bc my_slab(my_geom, my_bvals, stiff_params);
        my_slab.initialize_derivativesx(twodads::field_t::f_theta, t_src);

        // Initialize analytic solution for first x-derivative
        ...
        fname.str(string(""));
        fname << "test_derivs_ddx1_solan_" << Nx << "_device.dat";
        ...
        cuda_array_bc_nogp<twodads::real_t, allocator_device> sol_num(my_slab.get_array_ptr(twodads::field_t::f_theta_x));

        fname.str(string(""));
        fname << "test_derivs_ddx1_solnum_" << Nx << "_device.dat";
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // Initialize analytic solution for second x-derivative
        sol_an.apply([] __device__ (twodads::real_t dummy, size_t n, size_t m, twodads::slab_layout_t geom) -> twodads::real_t
            {
        ...
        // Write analytic solution to file
        fname.str(string(""));
        fname << "test_derivs_ddx2_solan_" << Nx << "_device.dat";
        ...
        fname.str(string(""));
        fname << "test_derivs_ddx2_solnum_" << Nx << "_device.dat";
        utility :: print(sol_num, t_src, fname.str());
        ...
        fname.str(string(""));
        fname << "test_derivs_ddy1_solnum_" << Nx << "_device.dat";
        ...
        // g_yy = 100 * (-1.0 + 100. * 8 * y) * exp(-50.0 * y * y)
        sol_an.apply([] __device__ (twodads::real_t dummy, size_t n, size_t m, twodads::slab_layout_t geom) -> twodads::real_t 
            {
                twodads::real_t y{geom.get_y(m)};
        ...
        // Write analytic solution to file
        fname.str(string(""));
        fname << "test_derivs_ddy2_solan_" << Nx << "_device.dat";
        utility :: print(sol_an, t_src, fname.str());
        ...
        fname.str(string(""));
        fname << "test_derivs_ddy2_solnum_" << Nx << "_device.dat";
        utility :: print(sol_num, 0, fname.str());

        sol_num -= sol_an;
        cout << "Second y-derivative: sol_num - sol_an: Nx = " << Nx << ", My = " << My << ", L2 = " << utility :: L2(sol_num, 0) << endl;
    }
}
