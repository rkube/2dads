...
namespace twodads {
    ...
    enum init_fun_t {init_NA, init_theta_gaussian, init_simple_sine, init_test, init_file};
    ...
    enum output_t {theta, theta_x, theta_y, omega, omega_x, omega_y, strmf};

    //typedef std::tuple<int, int> mode;
    struct stiff_params
    {
        twodads::real_t delta_t;
        twodads::real_t length_x;
        twodads::real_t length_y;
        twodads::real_t diff;
        twodads::real_t S;
        int level;
    };
