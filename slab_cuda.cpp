...
slab_cuda :: slab_cuda(slab_config my_config) :
    ...
    stiff_params{config.get_deltat(), config.get_lengthx(), config.get_lengthy(), config.get_model_param(0),
        4.0 * cuda::PI * cuda::PI * config.get_model_param(0) * config.get_deltat(), Nx, My / 2 + 1, tlevs},
    slab_layout{config.get_xleft(), config.get_deltax(), config.get_ylow(), config.get_deltay(), Nx, My}
{
    ...
    // Initialize block and grid sizes for arrays of size Nx * My/2+1
...
void slab_cuda :: initialize()
{
    ...
    switch(config.get_init_function())
    {
        ...
        case twodads::init_fun_t::init_theta_gaussian:
            ...
            d_dx(twodads::field_k_t::f_theta_hat, twodads::field_k_t::f_theta_x_hat);
            dft_c2r(twodads::field_k_t::f_theta_x_hat, twodads::field_t::f_theta_x, 0);

            d_dy(twodads::field_k_t::f_theta_hat, twodads::field_k_t::f_theta_y_hat);
            ...
            // Compute RHS
            cout << "theta_y_hat================================================\n";
            dump_field(twodads::field_k_t::f_theta_y_hat);
            rhs_fun();
